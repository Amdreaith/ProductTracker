  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (productError) {
        ...
        // Since the RPC approach is causing TypeScript errors, let's use a more direct approach
        // We'll handle the error differently
      ...
      if (price) {
        // Generate a unique ID for the price history entry
        const priceId = crypto.randomUUID();
        
        const { error: priceError } = await supabase
          .from('pricehist')
          .upsert({
            id: priceId, // Using built-in crypto.randomUUID instead of uuid package
            prodcode: sku,
